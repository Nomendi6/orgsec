...
- `JwtClaimsParser.getPositionRoleIds(String, Long)`. Read position roles from `parsePrincipalFromToken`. DTO accessors of the same name are unchanged.
- Deprecated placeholder types and factory methods on `StorageConfiguration` (`JwtSecurityDataStorage` / `RedisSecurityDataStorage` nested classes).
...
- Applications must run on Java 21 or newer. Java 17 remains supported on the OrgSec 1.1.x line (and on published 1.0.x until 1.1.0 supersedes it).
...
- Stop calling `JwtClaimsParser.getPositionRoleIds`. `orgsec.storage.primary`, `fallback`, `hybrid-mode-enabled` and `data-sources.*` still bind and stay inert.
...
Spring Boot 3.5 / Java 17 line (`release/1.1.0`). It is source- and binary-compatible with 1.0.5
for the japicmp-guarded public types. It keeps `includesDown` / `includesUp`,
`JwtClaimsParser.getPositionRoleIds`, the `StorageConfiguration` placeholder types, the 1.x
empty `privilegesList` aggregate fallback, and `=^*`. Redis RF, after-commit notify, and
optional ID-based `HIERARCHY_UP` land there first; the 2.0.0 port of those three is listed
under [2.0.0] above. 1.1.0 is not yet published to Central.
