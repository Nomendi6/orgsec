                ...
                for (Map.Entry<String, BusinessRoleDef> entry : organizationDef.businessRolesMap.entrySet()) {
                    ...
                    if (businessRoleDef.resourcesMap != null && !businessRoleDef.resourcesMap.isEmpty()) {
                        ...
                        if (privileges == null || privileges.isEmpty()) {
                            // Compatibility fallback for the 1.0.x line - see RsqlFilterBuilder for
                            // the same rule on the list path. Removed in 2.x, where this denies.
                            PrivilegeDef aggregate = privilegeChecker.getResourcePrivileges(resourceDef, operation);
                            if (aggregate == null || !privilegeChecker.hasRequiredOperation(aggregate, operation)) {
                                continue;
                            }
                            log.warn(
