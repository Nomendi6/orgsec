...
| 4a | `orgsec.storage.redis.enabled` and `orgsec.storage.features.redis-enabled` are set to the same value, and JWT is not enabled alongside Redis | Only the first activates Redis; the second decides whether the in-memory storage stands down from `@Primary`. Since 1.0.5 both a disagreement and the JWT+Redis combination refuse startup, so this is verified at first boot rather than in production - but verify it *before* the rollout window. | [Storage / Redis - Activation](../storage/03-redis.md#activation) |
...
| 11 | Redis `maxmemory-policy` is `noeviction` and memory fits two full snapshots | Eviction of a protocol key leaves GET/LIST deny until rebuild. | [Redis recovery](./redis-recovery.md) |
| 12 | Exactly one `SecurityDatasetFenceStore` and one `RedisSnapshotLoader`, with a stable `security-dataset-id` | A dependency-only upgrade from `<= 1.0.5` is refused; missing beans fail startup. | [Storage / Redis - Required beans](../storage/03-redis.md#required-application-beans) |
...
| 15 | Domain code calls `SecurityEventPublisher` after every authorization-relevant write, and the mutation increments the Redis dataset fence in the same database transaction | A bypassed fence or an in-transaction notify can publish the wrong generation or miss a rollback. `update*` is rejected on managed Redis. | [Usage / Load security data](../usage/08-load-security-data.md) |
...
| 19 | Redis is a dedicated standalone primary, not Cluster/Sentinel/replicas | Those topologies are not in the 1.1 contract. | [Storage / Redis - Supported topology](../storage/03-redis.md#supported-topology) |
| 20 | Documented runbook for NOT_READY / empty Redis | Rebuild from the source database; do not hand-edit protocol keys. | [Redis recovery](./redis-recovery.md) |
...
- [ ] 4a. Redis activation flags agree; JWT not enabled with Redis
