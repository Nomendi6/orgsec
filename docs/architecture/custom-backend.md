...
This is an advanced extension point. Most applications should first choose between [in-memory](../storage/02-in-memory.md), [Redis](../storage/03-redis.md), [JWT](../storage/04-jwt.md), or [hybrid](../storage/05-hybrid.md) storage.

OrgSec ships three storage backends - in-memory, Redis, and JWT - that cover the common cases. If your deployment needs something else (a JDBC-backed cache shared across instances, a MongoDB store, a process-local cache that loads from a microservice over gRPC), you implement `SecurityDataStorage` and provide a Spring Boot auto-configuration that wires it. This page is the advanced implementation guide for that.
...
2. **Does `updateXxx` write to your backend?** If yes, application code can use it for immediate-freshness flows (see [Usage / Load security data](../usage/08-load-security-data.md)). If no, leave the default implementation that throws `UnsupportedOperationException`.
...
- [Storage / Overview](../storage/01-choose-storage.md) - the user-facing comparison your backend will join.
