...
0. **Storage activation check (1.0.5+).** `OrgsecStorageActivationValidator`, an `EnvironmentPostProcessor`, reads the raw `Environment` before any bean definition exists and refuses startup on contradictory Redis flags, on a backend enabled without its module, and on JWT and Redis enabled together. Each failure names a stable `ORGSEC_STORAGE_*` code. It runs this early precisely because a contradiction otherwise surfaces as `NoUniqueBeanDefinitionException` during step 4, with no indication of which properties caused it.
