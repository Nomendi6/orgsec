...
0. **Storage activation check (1.0.5+).** `OrgsecStorageActivationValidator`, an `EnvironmentPostProcessor`, reads the raw `Environment` before any bean definition exists. Contradictory Redis flags are warned about (or refused, under `orgsec.storage.strict-activation: true`), and `jwt-enabled: true` without `orgsec-storage-jwt` on the classpath aborts here. It runs this early precisely because a contradiction otherwise surfaces as `NoUniqueBeanDefinitionException` during step 4, with no indication of which properties caused it.
