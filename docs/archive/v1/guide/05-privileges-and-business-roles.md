...
> **Note on `EXECUTE`.** `PrivilegeOperation.EXECUTE` is an independent operation. A privilege built with `EXECUTE` satisfies `hasRequiredOperation(..., EXECUTE)`, but it does not imply `READ` or `WRITE`; `READ` and `WRITE` also do not imply `EXECUTE`.
