...
For backend-specific behavior - how data is loaded, when caches are invalidated, what happens on cache miss - see the [Storage Overview](../storage/01-overview.md) and the per-backend pages.
...
- [Configuration](./04-configuration.md) - turn this model into `application.yml`.
- [Privileges and Business Roles](./05-privileges-and-business-roles.md) - design real privilege sets.
